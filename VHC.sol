 ...
 */
contract HINTBaseToken is DelayLockableToken {
    event HINTTransfer(address indexed from, address indexed to, uint256 value, string note);
    event HINTTransferFrom(address indexed owner, address indexed spender, address indexed to, uint256 value, string note);
    event HINTApproval(address indexed owner, address indexed spender, uint256 value, string note);

    event HINTMintTo(address indexed controller, address indexed to, uint256 amount, string note);
    event HINTBurnFrom(address indexed controller, address indexed from, uint256 value, string note);

    event HINTBurnWhenMoveToMainnet(address indexed controller, address indexed from, uint256 value, string note);

    event HINTSell(address indexed owner, address indexed spender, address indexed to, uint256 value, string note);
    event HINTSellByOtherCoin(address indexed owner, address indexed spender, address indexed to, uint256 value,  uint256 processIdHash, uint256 userIdHash, string note);

    event HINTTransferToTeam(address indexed owner, address indexed spender, address indexed to, uint256 value, string note);
    event HINTTransferToPartner(address indexed owner, address indexed spender, address indexed to, uint256 value, string note);

    event HINTTransferToEcosystem(address indexed owner, address indexed spender, address indexed to, uint256 value, uint256 processIdHash, uint256 userIdHash, string note);
    event HINTTransferToBounty(address indexed owner, address indexed spender, address indexed to, uint256 value, uint256 processIdHash, uint256 userIdHash, string note);

    // ERC20 함수들을 오버라이딩하여 super 로 올라가지 않고 무조건 hint~ 함수로 지나가게 한다.
    function transfer(address to, uint256 value) public returns (bool ret) {
        return hintTransfer(to, value, "");
    }

    function hintTransfer(address to, uint256 value, string note) public returns (bool ret) {
        require(to != address(this), "The receive address is the Contact Address of HINTToken. You cannot send money to this address.");

        ret = super.transfer(to, value);
        emit HINTTransfer(msg.sender, to, value, note);
    }

    function transferFrom(address from, address to, uint256 value) public returns (bool) {
        return hintTransferFrom(from, to, value, "");
    }

    function hintTransferFrom(address from, address to, uint256 value, string note) public returns (bool ret) {
        require(to != address(this), "The receive address is the Contact Address of HINTToken. You cannot send money to this address.");

        ret = super.transferFrom(from, to, value);
        emit HINTTransferFrom(from, msg.sender, to, value, note);
    }

    function approve(address spender, uint256 value) public returns (bool) {
        return hintApprove(spender, value, "");
    }

    function hintApprove(address spender, uint256 value, string note) public returns (bool ret) {
        ret = super.approve(spender, value);
        emit HINTApproval(msg.sender, spender, value, note);
    }

    function increaseApproval(address spender, uint256 addedValue) public returns (bool) {
        return hintIncreaseApproval(spender, addedValue, "");
    }

    function hintIncreaseApproval(address spender, uint256 addedValue, string note) public returns (bool ret) {
        ret = super.increaseApproval(spender, addedValue);
        emit HINTApproval(msg.sender, spender, allowed[msg.sender][spender], note);
    }

    function decreaseApproval(address spender, uint256 subtractedValue) public returns (bool) {
        return hintDecreaseApproval(spender, subtractedValue, "");
    }

    function hintDecreaseApproval(address spender, uint256 subtractedValue, string note) public returns (bool ret) {
    ...
    function hintMintTo(address to, uint256 amount, string note) onlyOwner public returns (bool ret) {
        ret = mintTo(to, amount);
        emit HINTMintTo(msg.sender, to, amount, note);
    }
    ...
    function hintBurnFrom(address from, uint256 value, string note) onlyOwner public returns (bool ret) {
        ret = burnFrom(from, value);
        emit HINTBurnFrom(msg.sender, from, value, note);
    }

    /**
     * dev 메인넷으로 이동하며 화폐 소각.
     */
    function hintBurnWhenMoveToMainnet(address burner, uint256 value, string note) onlyOwner public returns (bool ret) {
        ret = hintBurnFrom(burner, value, note);
        emit HINTBurnWhenMoveToMainnet(msg.sender, burner, value, note);
    }

    function hintBatchBurnWhenMoveToMainnet(address[] burners, uint256[] values, string note) onlyOwner public returns (bool ret) {
        uint256 length = burners.length;
        require(length == values.length, "The size of \'burners\' and \'values\' array is different.");

        ret = true;
        for (uint256 i = 0; i < length; i++) {
            ret = ret && hintBurnWhenMoveToMainnet(burners[i], values[i], note);
        }
    }

    /**
     * dev 이더로 HINT 를 구입하는 경우
     */
    function hintSell(
        address from,
        address to,
        ...
        ret = hintTransferFrom(from, to, value, note);
        emit HINTSell(from, msg.sender, to, value, note);
    }

    /**
     * dev 비트코인 등의 다른 코인으로 HINT 를 구입하는 경우
     * dev EOA 가 트랜잭션을 일으켜서 처리해야 하기 때문에 다계좌를 기준으로 한다. (가스비 아끼기 위함)
     */
    function hintBatchSellByOtherCoin(
        address from,
        address[] to,
        uint256[] values,
        ...
        ret = true;
        for (uint256 i = 0; i < length; i++) {
            require(to[i] != address(this), "The receive address is the Contact Address of HINTToken. You cannot send money to this address.");

            ret = ret && hintTransferFrom(from, to[i], values[i], note);
            emit HINTSellByOtherCoin(from, msg.sender, to[i], values[i], processIdHash, userIdHash[i], note);
        }
    }
    ...
    function hintTransferToTeam(
        ...
        ret = hintTransferFrom(from, to, value, note);
        emit HINTTransferToTeam(from, msg.sender, to, value, note);
    }

    /**
     * dev 파트너 및 어드바이저에게 전송하는 경우
     */
    function hintTransferToPartner(
        address from,
        address to,
        uint256 value,
        string note
        ...
        ret = hintTransferFrom(from, to, value, note);
        emit HINTTransferToPartner(from, msg.sender, to, value, note);
    }

    /**
     * dev 에코시스템(커뮤니티 활동을 통한 보상 등)으로 HINT 지급
     * dev EOA 가 트랜잭션을 일으켜서 처리해야 하기 때문에 다계좌를 기준으로 한다. (가스비 아끼기 위함)
     */
    function hintBatchTransferToEcosystem(
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            ret = ret && hintTransferFrom(from, to[i], values[i], note);
            emit HINTTransferToEcosystem(from, msg.sender, to[i], values[i], processIdHash, userIdHash[i], note);
        }
    }

    /**
     * dev 바운티 참여자에게 HINT 지급
     * dev EOA 가 트랜잭션을 일으켜서 처리해야 하기 때문에 다계좌를 기준으로 한다. (가스비 아끼기 위함)
     */
    function hintBatchTransferToBounty(
        address from,
        address[] to,
        uint256[] values,
        uint256 processIdHash,
        ...
        for (uint256 i = 0; i < length; i++) {
            ...
            ret = ret && hintTransferFrom(from, to[i], values[i], note);
            emit HINTTransferToBounty(from, msg.sender, to[i], values[i], processIdHash, userIdHash[i], note);
        }
    }
 ...
 * @title HINTToken
 */
contract HINTToken is HINTBaseToken {
    using AddressUtils for address;

    event TransferedToHINTDapp(
        address indexed owner,
        address indexed spender,
        address indexed to, uint256 value, HINTReceiver.HINTReceiveType receiveType);
    ...
    function hintTransfer(address to, uint256 value, string note) public returns (bool ret) {
        ret = super.hintTransfer(to, value, note);
        postTransfer(msg.sender, msg.sender, to, value, HINTReceiver.HINTReceiveType.HINT_TRANSFER);
    }

    function hintTransferFrom(address from, address to, uint256 value, string note) public returns (bool ret) {
        ret = super.hintTransferFrom(from, to, value, note);
        postTransfer(from, msg.sender, to, value, HINTReceiver.HINTReceiveType.HINT_TRANSFER);
    }

    function postTransfer(address owner, address spender, address to, uint256 value, HINTReceiver.HINTReceiveType receiveType) internal returns (bool) {
        if (to.isContract()) {
            bool callOk = address(to).call(bytes4(keccak256("onHINTReceived(address,address,uint256,uint8)")), owner, spender, value, receiveType);
            if (callOk) {
                emit TransferedToHINTDapp(owner, spender, to, value, receiveType);
            }
        }
    ...
    function hintMintTo(address to, uint256 amount, string note) onlyOwner public returns (bool ret) {
        ret = super.hintMintTo(to, amount, note);
        postTransfer(0x0, msg.sender, to, amount, HINTReceiver.HINTReceiveType.HINT_MINT);
    }

    function hintBurnFrom(address from, uint256 value, string note) onlyOwner public returns (bool ret) {
        ret = super.hintBurnFrom(from, value, note);
        postTransfer(0x0, msg.sender, from, value, HINTReceiver.HINTReceiveType.HINT_BURN);
    }
}


/**
 * @title HINTToken Receiver
 */
contract HINTReceiver {
    enum HINTReceiveType { HINT_TRANSFER, HINT_MINT, HINT_BURN }
    function onHINTReceived(address owner, address spender, uint256 value, HINTReceiveType receiveType) public returns (bool);
}

/**
 * @title HINTDappSample
 */
contract HINTDappSample is HINTReceiver {
    event LogOnReceiveHINT(string message, address indexed owner, address indexed spender, uint256 value, HINTReceiveType receiveType);

    function onHINTReceived(address owner, address spender, uint256 value, HINTReceiveType receiveType) public returns (bool) {
        emit LogOnReceiveHINT("I receive HINT Token.", owner, spender, value, receiveType);
        return true;
    }
}
